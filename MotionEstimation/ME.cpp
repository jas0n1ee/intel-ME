...
void PyramidME_4k(void *ref,void *src, std::vector<MotionVector> &MVs,std::vector<MotionVector> &ref_MV,std::vector<MotionVector>& MV_1080p,picinfo info,int Layers,int *cost,int lam1,int lam2,bool EnPreMV)
{
	std::vector<MotionVector> MV;
	std::vector<MotionVector> MV_sub;
	std::vector<MotionVector> MV_d2;
	...
	if(Layers>=2)
	{
		...
		if(Layers==3)
		{
			...
			if(EnPreMV)
				...
				compare_weak(ref_l[2]->Y,src_l[2]->Y,MV,picinfo(w/2,h/2),NULL,lam1);
			d4.resampling(MV,MV_sub);
		}
		else d4.costfunction(ref_l[1]->Y,src_l[1]->Y,MV_sub,MV_d4);
		if(EnPreMV)
	...
	h=height/pow(2,1);
	if(EnPreMV)
		compare(ref_l[0]->Y,src_l[0]->Y,MV_1080p,MV_sub,picinfo(w,h),NULL,lam1);
	else
		compare_weak(ref_l[0]->Y,src_l[0]->Y,MV_1080p,picinfo(w,h),NULL,lam1);
	me.resampling(MV_1080p,MV_sub);
	compare(ref,src,MVs,MV_sub,picinfo(width,height),cost,lam2);
	YUVUtils::ReleaseImage(src_l[0]);
	YUVUtils::ReleaseImage(ref_l[0]);
}
